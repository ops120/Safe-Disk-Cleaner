...
# 主程序逻辑
# ==========================================
class RiskAwareCleanerApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Win10 C盘深度清理专家 V3.0 (含风险评估)")
    ...
    def open_selected_folder(self):
        ...
        if tree:
            ...
            if selected:
                # 兼容两个列表不同的索引位置，路径总是在 values 的某个位置
                # junk: check, risk, category, path, ... (path is index 3)
                # large: check, risk, name, path, ... (path is index 3)
    ...
    def setup_clean_tab(self):
        ...
        self.tree_junk.column("risk", width=80, anchor="center") # 风险列
    ...
    def run_junk_scan(self):
        ...
        # 格式: (Category, Name, Path, DefaultCheck, RiskLevel)
        targets = [
        ...
        for i, (cat, name, path, default, risk) in enumerate(targets):
            ...
            if path and os.path.exists(path):
                ...
                if size > 0:
                    ...
                    self.tree_junk.insert("", "end", values=(mark, risk, cat, path, format_size(size), "待清理"))
                    total_size += size
            self.progress['value'] = (i + 1) / len(targets) * 100
        
        self.lbl_status.config(text=f"垃圾扫描完成，发现 {format_size(total_size)}")
        self.is_working = False
    ...
    def setup_large_tab(self):
        ...
        tk.Button(ctrl_frame, text="🔍 搜索", command=self.start_large_scan, bg="#FF9800", fg="white", padx=10).pack(side="left", padx=10)
        ...
        self.tree_large.column("risk", width=80, anchor="center") # 风险列
    ...
    def run_large_scan(self, start_path, limit_mb):
        ...
        try:
            for root, dirs, files in os.walk(start_path):
                if "Windows" in root and "WinSxS" in root: continue # 跳过WinSxS防止过慢
                ...
                for name in files:
                    try:
                        ...
                        if fsize > limit_bytes:
                            ...
                            self.tree_large.insert("", "end", values=("☐", risk, name, filepath, format_size(fsize), ext))
                            count += 1
                    except: pass
        except: pass
        ...
        self.lbl_status.config(text=f"搜索完成，找到 {count} 个文件")
        self.is_working = False
    ...
    def _start_generic_clean(self, tree_widget, mode):
        ...
        for item_id in tree_widget.get_children():
            ...
            if val[0] == "☑":
                # index 3 is path in both tabs now
    ...
    def run_generic_clean(self, tree, items, backup_dir, mode):
        ...
        for i, (item_id, path) in enumerate(items):
            ...
            try:
                ...
                vals[0] = "☐"
                vals[4] = "0 KB" # Size col index
                vals[5] = "已清理" # Status col index
                if mode == "large":
                     tree.delete(item_id)
                     continue
