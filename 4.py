...
# 主程序逻辑
# ==========================================
class ColorRiskCleanerApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Win10 C盘深度清理专家 V4.0 (彩色风险提示 + 停止功能)")
    ...
    def setup_clean_tab(self):
        ...
        self.btn_scan_junk = tk.Button(action_frame, text="🔍 扫描垃圾", command=self.start_junk_scan, bg="#0078D7", fg="white", padx=15)
        self.btn_scan_junk.pack(side="left", padx=5)
        
        self.btn_stop_junk = tk.Button(action_frame, text="🛑 停止", command=self.stop_current_action, state="disabled", padx=10)
        ...
        # 增加 risk 列
        columns = ("check", "risk", "category", "path", "size", "status")
        self.tree_junk = ttk.Treeview(self.tab_clean, columns=columns, show="headings")
        
        self.tree_junk.heading("check", text="选")
        self.tree_junk.heading("risk", text="风险")
        ...
        self.tree_junk.column("check", width=40, anchor="center")
        self.tree_junk.column("risk", width=80, anchor="center")
        self.tree_junk.column("category", width=80, anchor="center")
        self.tree_junk.column("path", width=450)
        self.tree_junk.column("size", width=80, anchor="e")
        self.tree_junk.column("status", width=80, anchor="center")
        
        # 配置颜色Tag
        self.tree_junk.tag_configure('safe', foreground='#2E7D32') # 绿色
        self.tree_junk.tag_configure('warn', foreground='#E65100') # 深橙色 (比纯黄易读)
        self.tree_junk.tag_configure('danger', foreground='#D32F2F') # 红色
    ...
    def start_junk_scan(self):
        ...
        self.btn_scan_junk.config(state="disabled")
        self.btn_stop_junk.config(state="normal")
    ...
    def run_junk_scan(self):
        ...
        total_size = 0
        for i, (cat, name, path, default, risk) in enumerate(targets):
            if self.stop_event: break # 停止检查
            ...
            if path and os.path.exists(path):
                ...
                if size > 0:
                    ...
                    self.tree_junk.insert("", "end", values=(mark, risk, cat, path, format_size(size), "待清理"), tags=(tag,))
                    total_size += size
            self.progress['value'] = (i + 1) / len(targets) * 100
        
        status_text = "扫描已停止" if self.stop_event else f"扫描完成，发现 {format_size(total_size)}"
        self.lbl_status.config(text=status_text)
        self.is_working = False
        self.btn_scan_junk.config(state="normal")
        self.btn_stop_junk.config(state="disabled")
    ...
    def setup_large_tab(self):
        ...
        tk.Label(ctrl_frame, text="最小(MB):").pack(side="left", padx=5)
        self.entry_size = tk.Entry(ctrl_frame, width=6)
        self.entry_size.insert(0, "100")
        self.entry_size.pack(side="left")

        tk.Label(ctrl_frame, text="路径:").pack(side="left", padx=5)
        self.entry_path = tk.Entry(ctrl_frame, width=25)
        self.entry_path.insert(0, os.path.expanduser("~"))
        self.entry_path.pack(side="left")
        
        tk.Button(ctrl_frame, text="...", command=lambda: self.select_search_path(), width=3).pack(side="left")
        
        self.btn_scan_large = tk.Button(ctrl_frame, text="🔍 搜索", command=self.start_large_scan, bg="#FF9800", fg="white", padx=10)
        self.btn_scan_large.pack(side="left", padx=10)
        
        self.btn_stop_large = tk.Button(ctrl_frame, text="🛑 停止", command=self.stop_current_action, state="disabled", padx=10)
        ...
        # 增加 risk 列
        columns = ("check", "risk", "name", "path", "size", "type")
        self.tree_large = ttk.Treeview(self.tab_large, columns=columns, show="headings")
        
        self.tree_large.heading("check", text="选")
        self.tree_large.heading("risk", text="风险")
        ...
        self.tree_large.column("check", width=40, anchor="center")
        self.tree_large.column("risk", width=80, anchor="center")
        self.tree_large.column("name", width=150)
        self.tree_large.column("path", width=400)
        self.tree_large.column("size", width=80, anchor="e")
        self.tree_large.column("type", width=60, anchor="center")
    ...
    def start_large_scan(self):
        ...
        self.is_working = True
        self.stop_event = False
        self.btn_scan_large.config(state="disabled")
        self.btn_stop_large.config(state="normal")
    ...
    def get_risk_level(self, filepath, ext):
        path_lower = filepath.lower()
        ext = ext.lower()
        
        if r"c:\windows" in path_lower: return "🔴 高"
        
        sys_exts = ['.sys', '.dll', '.exe', '.bat', '.cmd', '.vhdx', '.vhd', '.avhd']
        if ext in sys_exts: return "🔴 高"
        
        mid_exts = ['.msi', '.iso', '.wim']
        if ext in mid_exts: return "🟡 中"
    ...
    def run_large_scan(self, start_path, limit_mb):
        ...
        count = 0
        self.lbl_status.config(text=f"搜索中 (> {limit_mb}MB)...")

        try:
            for root, dirs, files in os.walk(start_path):
                if self.stop_event: break # 停止外层循环
                if "Windows" in root and "WinSxS" in root: continue
                
                for name in files:
                    if self.stop_event: break # 停止内层循环
                    try:
                        filepath = os.path.join(root, name)
                        fsize = os.path.getsize(filepath)
                        if fsize > limit_bytes:
                            ext = os.path.splitext(name)[1]
                            risk = self.get_risk_level(filepath, ext)
                            
                            tag = 'safe'
                            if "高" in risk: tag = 'danger'
                            elif "中" in risk: tag = 'warn'
                            
                            self.tree_large.insert("", "end", values=("☐", risk, name, filepath, format_size(fsize), ext), tags=(tag,))
                            count += 1
                    except: pass
        except: pass
        ...
        status_text = "搜索已停止" if self.stop_event else f"搜索完成，找到 {count} 个文件"
        self.lbl_status.config(text=status_text)
        
        self.is_working = False
        self.btn_scan_large.config(state="normal")
        self.btn_stop_large.config(state="disabled")
    ...
    def _start_generic_clean(self, tree_widget, mode):
        ...
        self.is_working = True
        # 清理时不应该被停止打断，因为可能造成文件损坏，所以不加 Stop 检查
    ...
    def get_folder_size(self, path):
        ...
        try:
            with os.scandir(path) as it:
                for entry in it:
                    if self.stop_event: break # 计算大文件夹时也允许停止
